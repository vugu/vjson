...
func readToken(r unreader) (Token, error) {
	...
	case 'n':
		...
		if !bytesEqual([]byte("ull"), buf[:]) {
			return nil, fmt.Errorf("expected `null` but got %q followed by %q", c, buf[:n])
		}
		return nil, nil
	...
	case 't':
		...
		if !bytesEqual([]byte("rue"), buf[:]) {
			return nil, fmt.Errorf("expected `true` but got %q followed by %q", c, buf[:n])
		}
		return true, nil
	...
	case 'f':
		...
		if !bytesEqual([]byte("alse"), buf[:]) {
			return nil, fmt.Errorf("expected `false` but got %q followed by %q", c, buf[:n])
		}
		return false, nil
	...
	case '"':
		...
		if foundEsc {
			ret, ok := unquote(buf)
			if !ok {
				return nil, fmt.Errorf("error unescaping JSON literal: %s", buf)
			}
			return ret, nil
		}
	...
	numread:
		for {
			...
			default:
				...
				err := r.UnreadByte()
				if err != nil {
					return nil, fmt.Errorf("error during UnreadByte(): %w", err)
				}
				break numread
			}
		}
	...
	return nil, fmt.Errorf("unexpected character while looking for JSON token start: %q", c)
...
// // canFloat64Exact returns true if the result of calling Float64() converts back to this string exactly
// func (n Number) canFloat64Exact() bool {
// }
